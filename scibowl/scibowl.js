...
function updateScores() {
    ...
    for (let ii = 0; ii < numQuestions; ii++) {
        ...
        if (lscore > rscore && ii <= latestClicked) {
            $($(tLeftArray[i]).children("p").get(1)).css("background-color", "rgb(150, 255, 150)");
        } else {
            $($(tLeftArray[i]).children("p").get(1)).css("background-color", "");
        }
        if (lscore < rscore && ii <= latestClicked) {
            $($(tRightArray[i]).children("p").get(0)).css("background-color", "rgb(150, 255, 150)");
        } else {
...
function setup() {
    ...
    document.documentElement.style.setProperty("--number-buttons", numQuestions);
