const exp     = require('constants');
const express = require('express');
const fs      = require('fs');
const parse   = require('path');
const os      = require("os");
const path    = require('path');
const bodyParser = require("body-parser");

const app = express()
const port = 3000

//app.use(bodyParser.json());
var jsonParser = bodyParser.json();

var pageHierarchy   = {};
var listOfEquations = [];

var namePageNlink = {} // Table of name-absolute path links for name-links to use in redirects

var allEquationsText = `PageType:Page
PageDisplayTitle:All Equations
PageDescription:
SubtopicName:Other
Categories:`;

class PageClass {
    type;
    name;
    fancyName;
    description;
    subtopics;
    fullText;
    categories;

    constructor() {
        this.type         = "page";
        this.name         = "";
        this.fancyName    = "";
        this.description  = "";
        this.fullText     = "";
        this.subtopics    = [];
        this.categories   = [];
    }
}

class EquationClass {
    type;
    name;
    fancyName;
    equationText;
    categories;
    fullText;

    constructor() {
        this.type         = "equation";
        this.name         = "";
        this.fancyName    = "";
        this.equationText = "";
        this.fullText     = "";
        this.categories   = [];
    }
}

var wildcardString = os.hostname().includes("alex") ? "*" : "{*any}";
var ogText = fs.readFileSync("./astro/reader.html").toString();

app.get('/', (req, res) => {
    res.redirect("/topics/");
    // TODO : Implement actual home screen
});

app.get("/favicon.png", (req, res) => {
    res.sendFile("./favicon.png", {root: __dirname});
});

app.get("/favicon.ico", (req, res) => {
    res.sendFile("./favicon.png", {root: __dirname});
});

app.get("/reader.css", (req, res) => {
    res.type("text/css");
    res.sendFile("./reader.css", {root: __dirname});
});

app.get("/reader.js", (req, res) => {
    res.type("text/javascript");
    res.sendFile("./reader.js", {root: __dirname});
});

app.get("/topics/" + wildcardString, (req, res) => {
    if(!req.originalUrl.endsWith("/") && false) {
        res.redirect(req.originalUrl + "/"); 
    } else {
        var dat = getFileData(req.path);
        let newText = ogText.replace("<!-- REPLACE ME SERVER -->", JSON.stringify(dat).replaceAll("\\", "\\\\"));
        res.type("html");
        res.send(newText);
    }
});

app.get("/ComputerModernSerif.ttf", (req, res) => {
    res.sendFile("./fonts/cmunrm.ttf", {root: __dirname});
});

app.get("/jquery.js", (req, res) => {
    res.sendFile("jquery-3.7.1.min.js", {root: __dirname});
});

app.get("/images/" + wildcardString, (req, res) => {
    var imageName = decodeURI(req.originalUrl).split("/").pop();
    if(fs.existsSync(__dirname + "/staticimages/images/" + imageName)) {
        res.sendFile("./staticimages/images/" + imageName, {root: __dirname});
    } else {
        res.status(404).send("Sorry not found :(");
    }
});

app.get("/nlink/" + wildcardString, (req, res) => {
    var nLinkPageName = decodeURI(req.originalUrl.slice(7));
    var subpage = "";
    if(nLinkPageName.includes("/")) {
        subpage = nLinkPageName.split("/")[1];
        nLinkPageName = nLinkPageName.split("/")[0];
    }
    if(nLinkPageName in namePageNlink) {
        res.redirect(namePageNlink[nLinkPageName] + (subpage == "" ? "" : "/" + subpage));
    } else {
        res.send("Page not found");
    }
});

app.post("/api/search", jsonParser, (req, res) => {
    if(!"searchTerm" in req.body) {
        res.send({success : false, message : "No search term sent!"});
        return;
    }
    let searchTerm = req.body.searchTerm.toLowerCase();
    let toReturnTemp = [];
    for(const eq of listOfEquations) {
        if(eq.fancyName.toLowerCase().includes(searchTerm) || eq.fullText.toLowerCase().includes(searchTerm) || eq.categories.some((cat) => {return cat.toLowerCase().includes(searchTerm)})) {
            toReturnTemp.push([eq.fancyName, eq.equationText, eq.name]);
            if(toReturnTemp.length >= 5) break;
        }
    };
    res.send({
        success : true,
        searchResults : toReturnTemp
    });
});

var currentPage;

function getFileData(ogPathThing) {
    var pathTrace = ogPathThing.split("/");
    if(pathTrace[pathTrace.length-1] == "") pathTrace.splice(pathTrace.length-1, 1);
    if(pathTrace[0] == "") pathTrace.splice(0, 1);

    if(pathTrace.length == 1) { // On the home page
        return {
            success        : true,
            type           : "home",
            randomEquation : listOfEquations[Math.floor(Math.random() * listOfEquations.length)],
            fullText       : pageHierarchy.fullText
        }
    }

    currentPage = pageHierarchy;
    var stackTrace = [];
    var stackTraceUrls = [];

    var i = 0;
    while(++i < pathTrace.length) {
        var found = false;
        for(var ii = 0; ii < currentPage.subtopics.length; ii++) {
            var page = currentPage.subtopics[ii];
            if(page.name == pathTrace[i]) {
                currentPage = page;

                // stackTrace.push(page.fancyName);
                // stackTraceUrls.push(page.name);

                found = true;
                break;
            }
        }

        if(found) continue;
        
        if(pathTrace[pathTrace.length-2] == currentPage.name) break;

        return { // Subpage not found
            success : false,
            code    : 404,
            temp    : currentPage.name
        };
    }

    if(!found) return { // Subpage not found
        success : false,
        code    : 404,
        temp    : currentPage.name
    };

    if(stackTrace.length) stackTrace.pop();

    var toSend = {};

    if(currentPage.type == "page") {
        toSend = {
            success        : true,
            type           : "page",
            name           : currentPage.name,
            fancyName      : currentPage.fancyName,
            fullText       : currentPage.fullText,
            // subtopics      : [],
        };
    } else if(currentPage.type == "equation") {
        toSend = {
            success        : true,
            type           : "equation",
            name           : currentPage.name,
            fancyName      : currentPage.fancyName,
            equation       : currentPage.equationText,
            fullText       : currentPage.fullText,
            // subtopics      : [],
        };
    }

    // for(var i = 0; i < currentPage.subtopics.length; i++) {
    //     toSend.subtopics.push({
    //         name         : currentPage.subtopics[i].name,
    //         fancyName    : currentPage.subtopics[i].fancyName,
    //         description  : currentPage.subtopics[i].description,
    //         subtopicName : currentPage.subtopics[i].subtopicName,
    //     });
    // }

    return toSend;
};

function parseFile(path) {
    var txt = fs.readFileSync(path, {encoding: 'utf-8'}); // Parse index.rtd text
    return parseText(txt, path);
}

function parseText(txt, path) {
    var toReturn = null;

    var lines = txt.split("\n");

    let isEquation = false;

    if(lines[0] == "PageType:Equation") {
        toReturn = new EquationClass();
        lines.splice(i--, 1);
        isEquation = true;

        for(var i = 0; i < lines.length; i++) {
            if(lines[i].startsWith("PageDisplayTitle")) {
                toReturn.fancyName = lines[i].split(":")[1];
                lines.splice(i--, 1);
                continue;
            } else if(lines[i].startsWith("MainEquation")) {
                toReturn.equationText = lines[i].split(":")[1];
                lines.splice(i--, 1);
                continue;
            } else if(lines[i].startsWith("Categories")) {
                toReturn.categories = lines[i].split(":")[1].split(",");
                lines.splice(i--, 1);
                continue;
            }
        }
    } else if(lines[0] == "PageType:Page") {
        toReturn = new PageClass();
        lines.splice(i--, 1);

        for(var i = 0; i < lines.length; i++) {
            if(lines[i].startsWith("PageDisplayTitle")) {
                toReturn.fancyName = lines[i].split(":")[1];
                lines.splice(i--, 1);
                continue;
            } else if(lines[i].startsWith("PageDescription")) {
                toReturn.description = lines[i].split(":")[1];
                lines.splice(i--, 1);
                continue;
            } else if(lines[i].startsWith("SubtopicName")) {
                toReturn.subtopicName = lines[i].split(":")[1];
                lines.splice(i--, 1);
                continue;
            } else if(lines[i].startsWith("Categories")) {
                toReturn.categories = lines[i].split(":")[1].split(",");
                lines.splice(i--, 1);
                continue;
            }
        }
    }

    var t = path.split("/");
    if(path.endsWith("index.rtd")) {
        toReturn.name = t[t.length-2];
    } else {
        toReturn.name = t[t.length-1].split(".")[0];
    }

    //console.log(toReturn.name);

    while(lines.size && lines[0].trim() == "") {
        lines.splice(0, 1);
    }

    toReturn.fullText = lines.join("\n");

    namePageNlink[toReturn.fancyName] = path.replace(".rtd", "").replace("/index", "").replace("/astro", "").substring(1);
    namePageNlink[toReturn.name] = path.replace(".rtd", "").replace("/index", "").replace("/astro", "").substring(1);

    if(isEquation) {
        listOfEquations.push(toReturn);
        allEquationsText += `
        __h2__${toReturn.fancyName}__
        \\[ ${toReturn.equationText} \\]
        __newl__`;
    }

    return toReturn;
}

function readOverFolder(path) { // ALL WORKING FILE PATHS MUST HAVE A TRAILING SLASH
    var toReturn;
    if(fs.existsSync(path + "index.rtd")) {
        toReturn = parseFile(path + "index.rtd");
    } else {
        return null; // Couldn't find index.rtd
    }
    fs.readdirSync(path).forEach(file => {
        if(file.endsWith("index.rtd")) {
            // Done
        } else if(!file.startsWith(".")) {
            if(fs.lstatSync(path + file).isDirectory()) {
                var t = readOverFolder(path + file + "/");
                if(t != null) toReturn.subtopics.push(t);
            } else if(file.endsWith(".rtd")) {
                toReturn.subtopics.push(parseFile(path + file))
            }
        }
    });
    return toReturn;
}

if(os.hostname().includes("alex")) app.get("/reloadList", (req, res) => {
    pageHierarchy = readOverFolder("./astro/topics/");
    console.log("Reloaded data files");
    res.send("Reloaded");
});

exports.app = app;

exports.startUpFunction = function() {
    console.log("Reading over astro data files...");
    pageHierarchy = readOverFolder(`./astro/topics/`);
    pageHierarchy.subtopics.push(parseText(allEquationsText, "/topics/alleqs"));
    //console.log(pageHierarchy);
}

exports.shutDownFunction = function() {

}
