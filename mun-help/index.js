...
app.get("/topics/" + wildcardString, (req, res) => {
    if(!req.originalUrl.endsWith("/")) {
        ...
        var ogText = fs.readFileSync("./mun-help/reader.html").toString();
...
function parseFile(path) {
    ...
    namePageNlink[toReturn.fancyName] = path.replace(".rtd", "").replace("/index", "").replace("/mun-help", "").substring(1);
    namePageNlink[toReturn.name] = path.replace(".rtd", "").replace("/index", "").replace("/mun-help", "").substring(1);
...
if(os.hostname().includes("alex")) app.get("/reloadList", (req, res) => {
    pageHierarchy = readOverFolder("./mun-help/topics/");
...
exports.startUpFunction = function() {
    console.log("Reading over mun-help data files...");
    pageHierarchy = readOverFolder(`./mun-help/topics/`);
}
